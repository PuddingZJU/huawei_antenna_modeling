...
void BVHWriter::setEulerAnglesSeq(EulerAnglesSeq seq)
{
	this->m_bChangeEulerAnglesSeq = true;
	this->m_EulerAnglesSeq = seq;
}
...
void BVHWriter::writeSkeleton()
{
	...
	if (m_bChangeEulerAnglesSeq)
	{
		seqnow = m_EulerAnglesSeq;
	}
	else
	{
		seqnow = this->m_pmc->getRoot(0)->getEulerAnglesSequence();
	}
